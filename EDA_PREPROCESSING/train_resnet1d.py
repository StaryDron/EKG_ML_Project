...
def run_epoch(model, loader, criterion, optimizer=None, device="cpu", n_classes=5):
    ...
    # zbieramy predykcje tylko dla eval (val/test)
    all_probs = []
    all_true = []
    ...
    for x, y in loader:
        ...
        # metryki AUC/PR/F1 liczymy sensownie na CAŁYM zbiorze (val/test),
        # więc zbieramy prawdopodobieństwa i etykiety
        if not train:
...
if __name__ == "__main__":
    ...
    for epoch in range(1, 8):
        ...
        if val_acc > best_val_acc:
            ...
            torch.save(model.state_dict(), os.path.join(ART_DIR, "resnet1d_best.pt"))

    # test na najlepszym
    model.load_state_dict(torch.load(os.path.join(ART_DIR, "resnet1d_best.pt"), map_location=device))
